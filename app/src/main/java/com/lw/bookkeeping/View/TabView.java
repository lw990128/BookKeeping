package com.lw.bookkeeping.View;
...
public class TabView extends FrameLayout {
    ...
    protected void onFinishInflate() {
        ...
        // 设置轮廓图片，不透明，默认着色为黑色
        ImageView mNormalImageView = findViewById(R.id.tab_image);
        mNormalDrawable.setTint(DEFAULT_TAB_COLOR);
        mNormalDrawable.setAlpha(255);
        mNormalImageView.setImageDrawable(mNormalDrawable);

        // 设置选中图片，透明， 默认着色为黑色
        ImageView mSelectedImageView = findViewById(R.id.tab_selected_image);
    ...
    public void setXPercentage(float percentage) {
        ...
        // 1. 颜色变换
        int finalColor = evaluate(percentage, mTargetColor);
     ...
     * @return 返回与进度值相应的颜色值。
     */
    private int evaluate(float percentage, int endValue) {
        float startA = ((TabView.DEFAULT_TAB_COLOR >> 24) & 0xff) / 255.0f;
        float startR = ((TabView.DEFAULT_TAB_COLOR >> 16) & 0xff) / 255.0f;
        float startG = ((TabView.DEFAULT_TAB_COLOR >>  8) & 0xff) / 255.0f;
        float startB = ( TabView.DEFAULT_TAB_COLOR & 0xff) / 255.0f;
